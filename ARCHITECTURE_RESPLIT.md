...
5. **2 iyzico merchant (NETLEŞTİ 2026-06-15):**
   - **brytakip.com → merchant `101029161`** (mevcut PROD; BRY sahipleniyor). BRY ürünü
     (`92a1409c` / 249₺ plan `dd82b6e9`) zaten burada → **yeni kurulum gerekmez.** Bu merchant'tan
     Studio+Atölye ürünleri **terk/sil**; merchant iyzico panelinde **brytakip.com**'a kayıtlanır;
     sızmış key'leri **rotate**.
   - **ludenlab.com (studio+atolye) → YENİ merchant** (kullanıcı başvuruyor; iyzico onayı
     **lead-time = kritik yol**, erken başlat). Onaya kadar tüm Track 2 dev/test **sandbox merchant
     `3422180`** ile. Yeni merchant **ludenlab.com**'a kayıtlanır.
   - Split iyzico'nun **"tek iş = tek domain"** kuralına tam uyar.
...
| Faz | İş | Kim | Risk |
|---|---|---|---|
| **B1** | `brytakip-api` sertleştir: `CENTRAL_BILLING` kapalı teyit; hardcoded `brytakip.ludenlab.com`→`brytakip.com` (`src/server.ts` CORS, `src/lib/mailer.ts`, `src/routes/license.ts` mesajları) + `BRYTAKIP_WEB_URL` env; iyzico merchant `101029161` (rotate edilmiş PROD key) + `IYZICO_PRICING_PLAN_REF=dd82b6e9…` (249₺) doğrula | ben | düşük |
...
| **B5** | Merkezden BRYTAKIP düş: hub `BillingModule` enum, `bootstrap-iyzico.mjs`, `createWebhookRouter` fan-out, `moduleReturnUrl`, `migrate-subscriptions.mjs`, `set-prod-plan-refs.mjs`'ten BRYTAKIP temizliği. **BRY ürün/planı `101029161`'de KALIR (dokunma);** o merchant'taki Studio+Atölye ürünleri Track 2/S0'da silinir | ben | düşük |
...
| Faz | İş | Kim | Risk |
|---|---|---|---|
| **S0** | **Yeni iyzico merchant başvurusu** (studio+atolye, ludenlab.com'a kayıtlı) — ⚠️ iyzico onay lead-time = KRİTİK YOL, **şimdi başlat**. Onaya kadar dev/test sandbox `3422180`. `101029161` BRY'ye kalır; oradaki Studio+Atölye ürünleri silinir | sen | düşük (lead-time) |
...
- ~~**BRY'nin "mevcut iyzico'su"**~~ **ÇÖZÜLDÜ (2026-06-15):** BRY = merchant `101029161` (mevcut
  PROD, artık brytakip.com'a özel). Studio+Atölye → yeni merchant (kullanıcı başvuruyor). Bkz. §2.5.
