...
import { davranisInputSchema } from "@/lib/davranis";
import { generateDavranis } from "@/lib/davranis-prompts";
...
export async function POST(req: Request) {
  ...
  try {
    const charged = await withCredits(session.user.id, () => generateDavranis(parsed.data));
    ...
    console.error("[atolye/davranis] üretim hatası", err);
    return NextResponse.json(
      { error: "Plan üretilemedi. Lütfen tekrar deneyin." },
      { status: 500 },
    );
  }
}
