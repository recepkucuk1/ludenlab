...
import { veliMektubuInputSchema } from "@/lib/veli-mektubu";
import { generateVeliMektubu } from "@/lib/veli-mektubu-prompts";
...
export async function POST(req: Request) {
  ...
  try {
    const result = await generateVeliMektubu(parsed.data);
    return NextResponse.json({
      text: result.text,
      model: result.model,
