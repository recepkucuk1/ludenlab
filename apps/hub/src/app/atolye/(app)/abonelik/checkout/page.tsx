...
import { PButton as PButton, PCard } from "@ludenlab/ui";
...
function CheckoutContent() {
  ...
  useEffect(() => {
    ...
    async function initializeCheckout() {
      try {
        const res = await fetch("/atolye/api/subscription/checkout", {
          method: "POST",
  ...
  if (error) {
    return (
      ...
      >
        <PCard  style={{ maxWidth: 440, width: "100%", padding: 32, textAlign: "center" }}>
          <h2
            style={{
              margin: "0 0 10px",
              fontSize: 20,
          ...
          <PButton onClick={() => router.push("/atolye/abonelik")} variant="accent" size="md" style={{ width: "100%" }}>
            ← Planlara Dön
          </PButton>
        </PCard>
      </div>
  ...
  return (
    <div className="poster-scope" style={{ maxWidth: 960, margin: "0 auto", padding: "40px 16px" }}>
      ...
      <PCard  style={{ padding: 28, position: "relative", minHeight: 400 }}>
        {loading && (
          <div
            style={{
              position: "absolute",
