...
import { initializeCheckoutForm } from "@atolye/lib/iyzico";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const dummyAddress = {
      contactName: account.name || "Terapist",
      city: "Istanbul",
      district: "Kadikoy",
      country: "Turkey",
    ...
    const checkoutFormRes = await initializeCheckoutForm({
      ...
      customer: {
        ...
        surname: account.name?.split(" ").slice(1).join(" ") || "Terapisti",
        identityNumber: "11111111111", // Required by standard flow, dummy in sandbox
        email: account.email,
        gsmNumber: formatGsmNumber(undefined) || "+905350000000",
