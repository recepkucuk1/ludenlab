...
import { CardPreview } from "@studio/components/cards/CardPreview";
import { AssignStudentsModal } from "@studio/components/cards/AssignStudentsModal";
import { SocialStoryView, type SocialStoryContent } from "@studio/components/cards/SocialStoryView";
import { ArticulationView, type ArticulationContent } from "@studio/components/cards/ArticulationView";
import { HomeworkView, type HomeworkContent } from "@studio/components/cards/HomeworkView";
import { SessionSummaryView, type SessionSummaryContent } from "@studio/components/cards/SessionSummaryView";
import { MatchingGameView, type MatchingGameContent } from "@studio/components/cards/MatchingGameView";
...
async function downloadPhonationPDF(card: CardRecord) {
  ...
  const renderContent = () => {
    ...
    if (aType === "snakes_ladders") {
      const grid = activity.grid as { rows: number; cols: number; cells: { position: number; word: string; isLadder?: boolean; isSnake?: boolean; imageUrl?: string }[] } | undefined;
      if (!grid) return null;
      const cells = Array.isArray(grid.cells) ? grid.cells : [];
      const total = cells.length;
      const gCols = grid.cols || 5;
      const gRows = grid.rows || Math.ceil(total / gCols);
      const boardRows: (typeof cells)[] = [];
      for (let r = 0; r < gRows; r++) {
        const rowCells = cells.filter((c) => c.position > r * gCols && c.position <= (r + 1) * gCols);
        boardRows.unshift((gRows - 1 - r) % 2 === 0 ? rowCells : [...rowCells].reverse());
      }
      const cellW = Math.floor(507 / gCols) - 3;
      return (
        <View>
    ...
    // ── SES LABİRENTİ ────────────────────────────────────────────────────────
    if (aType === "sound_maze") {
      const grid = activity.grid as { cols?: number; cells: { word: string; hasTargetSound: boolean; position?: number; imageUrl?: string }[] } | undefined;
      if (!grid) return null;
      const cells = Array.isArray(grid.cells) ? grid.cells : [];
  ...
  async function handleDownloadPDF() {
    ...
    try {
      ...
      if (tt === "SOCIAL_STORY") {
        ...
        await downloadWeeklyPlanPDF(
          { ...(card.content as unknown as WeeklyPlanContent), title: card.title },
          card.student?.name,
        );
      } else {
